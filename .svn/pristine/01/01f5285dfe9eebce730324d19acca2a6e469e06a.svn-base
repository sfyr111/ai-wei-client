  ...
  <div id="column-video">
    <div class="column-top">
      <video
        ...
        class="video-js"
        controls
        x-webkit-airplay="true" webkit-playsinline="" x5-playsinline="" playsinline="" x5-video-player-type="h5" x5-video-player-fullscreen="true"
        preload="auto"
        :poster="imgUrl"
        data-setup='{}' >
      ...
      <div class="operation">
        ...
        <div class="operation-right">
          ...
          <section v-show="isFavorite" class="isFavorite" @click="_deleteFavoriteColumn(columnId)">
...
<script>
  ...
  import { getFavoriteColumn, addFavoriteColumn, getCoursesByColumnId, getCoursesById, deleteFavoriteColumn, watchFrequency } from './api'
    ...
    methods: {
      play (obj) {
        if (this.videoUrl !== obj.url) {
          ...
          this._watchFrequency(obj.videoId)
        }
        if (this.videoUrl !== obj.url) {
          this.paused = true
        }
      ...
      async _watchFrequency (videoId) {
        const params = {}
        return watchFrequency(params, videoId).then(data => {
          if (data.code === ERR_OK) {
            return data
          }
        })
